...
const METADATA_REGEX = /(?:<!---\s*|\/\/\/\s*|###\s*)(?<key>file|link|copy):\s*(?<value>.*?)(?:\s*--->|$)\n/gm;
...
const original_file = './packages/ui-kit/src/lib/markdown/renderer.ts';
if (!fs.existsSync(original_file)) {
    throw new Error('Update the path to the markdown renderer code. Current value: ' +
        original_file +
        ' | Current cwd: ' +
        process.cwd());
}
 ...
 */
export async function render_content_markdown(filename, body, options, twoslashBanner) {
    const headings = [];
    const { check = true } = options ?? {};
    return await transform(body, {
        async walkTokens(token) {
            if (token.type === 'code') {
                if (snippets.get(token.text))
                    return;
                if (token.lang === 'diff') {
                ...
                source = source.replace(/(\+\+\+|---|:::)/g, (_, delimiter) => {
                    return delimiter_substitutes[delimiter];
                });
                const converted = token.lang === 'js' || token.lang === 'svelte'
            ...
            if (tokens) {
                ...
                const slash_index = tokens.findIndex((token) => token.type === 'text' && token.text === '/') ?? -1;
                if (slash_index !== -1) {
...
async function convert_to_ts(js_code, indent = '', offset = '') {
    ...
    async function walk(node) {
        ...
        if (jsdoc) {
            ...
            else if ((ts.isPropertyAssignment(node) && ts.isArrowFunction(node.initializer)) ||
    ...
    function get_jsdoc_type_expression_text(text) {
        return text.replace(/^@type\s*/, '').trim();
    }
}
function find_nearest_node_modules(file) {
    let current = file;
 ...
 */
function hash_graph(hash, file, seen = new Set()) {
    if (seen.has(file))
        return;
    seen.add(file);
...
function parse_options(source, language) {
...
async function syntax_highlight({ prelude, source, filename, language, check }) {
    let html = '';
    if (/^(dts|yaml|yml)/.test(language)) {
        html = replace_blank_lines(await codeToHtml(source, {
        ...
        try {
            ...
            if (check) {
                ...
                html = html.replace(/<div class="twoslash-meta-line twoslash-error-line">[^]+?<\/div>/g, '\n');
                const replacements = [];
                ...
                for (const match of html.matchAll(/<span class="twoslash-popup-docs-tag"><span class="twoslash-popup-docs-tag-name">([^]+?)<\/span><span class="twoslash-popup-docs-tag-value">([^]+?)<\/span><\/span>/g)) {
                    const tag = match[1];
                    let value = match[2];
...
function indent_multiline_comments(str) {
    return str.replace(/^(\s+)<span class="token comment">([\s\S]+?)<\/span>\n/gm, (_, intro_whitespace, content) => {
        ...
        return lines
            ...
            return `<span class="token comment wrapped" style="--indent: ${indent}ch">${line ?? ''}</span>`;
        })
            .join('');
    });
}
