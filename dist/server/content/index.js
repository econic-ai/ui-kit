...
export async function create_index(documents, assets, base, read) {
    const content = {};
    const roots = [];
    for (const key in documents) {
        ...
        if (metadata.description)
            metadata.description = smart_quotes(metadata.description).replace(/`(.+?)`/g, '<code>$1</code>');
...
function create_links(document, prev) {
    if (document.body) {
        link(prev, document);
        prev = document;
    }
...
function link(prev, next) {
    if (prev)
        prev.next = next && { slug: next.slug, title: next.metadata.title };
    if (next)
        next.prev = prev && { slug: prev.slug, title: prev.metadata.title };
}
