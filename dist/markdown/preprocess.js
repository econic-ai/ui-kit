 ...
 */
export async function preprocess(file, modules) {
    ...
    content = await async_replace(content, REGEXES.EXPANDED_TYPES, async ([_, name, id]) => {
        ...
        if (!type)
            throw new Error(`Could not find type ${name}#${id}`);
    ...
    content = await async_replace(content, REGEXES.EXPORT_SNIPPET, async ([_, name, id]) => {
        const module = modules.find((module) => module.name === name);
        if (!module)
            throw new Error(`Could not find module ${name} for EXPORT_SNIPPET clause`);
        if (!id) {
    ...
    content = await async_replace(content, REGEXES.MODULES, async () => {
        return modules
            ...
            if (!module.exports)
                return;
            if (module.exports.length === 0 && !module.exempt)
                return '';
            let import_block = '';
...
function render_declaration(declaration, full) {
    let content = '';
 ...
 */
function stringify_module(module) {
    let content = '';
...
function stringify_expanded_type(type) {
    return (type.comment +
        type.overloads
            .map((overload) => overload.children
            ?.map((child) => {
...
function stringify(member, lang = 'ts') {
    ...
    const bullet_block = (member.bullets?.length ?? 0) > 0
        ? `\n\n<div class="ts-block-property-bullets">\n\n${member.bullets?.join('\n')}\n\n</div>`
        : '';
    const comment = member.comment
        ...
        : '';
    const child_block = (member.children?.length ?? 0) > 0
        ? `\n\n<div class="ts-block-property-children">${member.children
            ?.map((val) => stringify(val, lang))
            .join('\n')}</div>`
        : '';
    return (`<div class="ts-block-property">${fence(member.snippet, lang)}` +
